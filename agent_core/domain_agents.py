...
AGENT_CONFIGS = {
    ...
    "knowledge": {
        ...
        "system_prompt": (
            ...
            "你的能力包括：\n"
            "- list_kb_entries: 列出知识库中的所有条目（用户问'有什么'时优先用这个）\n"
            "- search_knowledge: 用关键词搜索特定内容\n"
            "- list_kb_categories: 查看分类\n"
            "- create_kb_entry: 添加新条目\n"
            "行动准则：\n"
            "- 用户想浏览或了解知识库内容时，先用 list_kb_entries 列出条目，不要用 search_knowledge 瞎搜\n"
            "- 用户有明确搜索目标时，才用 search_knowledge\n"
            "- 基于搜索结果为用户总结关键信息\n"
            "- 添加知识时选择合适的分类\n"
        ),
